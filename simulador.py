...
class Barramento:
    ...
    def read(self, endereco, id):
        ...
        for cpu in self.cpus:
            ...
            if linha := cpu.privateCache[endereco]:
                match linha.estado:
        ...
        if bloco is None:
            if linha := self.sharedCache[endereco]:
                bloco = linha.bloco
...
class Cpu:
    ...
    def processar(self, instrucao, endereco):
        if linha := self.privateCache[endereco]:
        ...
        if instrucao == Instrucao.ESCRITA_DADO:
            if not (bloco := self.barramento.read_exclusive(endereco, self.id)):
                bloco = self.busca(endereco)
            self.insereM(bloco)
            return

        # instrucao == Leitura
        if not (bloco := self.barramento.read(endereco, self.id)):
